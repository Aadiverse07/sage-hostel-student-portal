...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "development-only-secret-change-me")

# Fixed administrator credentials. Student accounts continue to use their own
# registered email/password credentials.
ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "admin@sagehostel.in")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "SAGEAdmin@2026")
ADMIN_SESSION_KEY = os.environ.get("ADMIN_SESSION_KEY", "SAGE-ADMIN-2026")
DEMO_STUDENT_PASSWORD = os.environ.get("DEMO_STUDENT_PASSWORD", "sage123")

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATABASE = os.path.join(os.path.dirname(BASE_DIR), "hostel.db")
...
def initialize_database():
    ...
    """, (
        "230000001",
        generate_password_hash(DEMO_STUDENT_PASSWORD),
        "Aditya Singh",
        "B.Tech CSE",
        "1st Year",
        "118",
        "aditya.singh@sageuniversity.in",
        "7992294390",
    ...
    cursor.execute("""
        ...
        VALUES ('118', 'Ground Floor', 2, 'Wi-Fi, Study Table, Cupboard, Attached Bathroom', 'Occupied')
    """)
    ...
    student = cursor.execute(
        "SELECT id FROM students WHERE enrollment = ?", ("230000001",)
    ).fetchone()
...
def verify_payment(payment_id):
    ...
    conn.commit()
    conn.close()
    return redirect(url_for("admin_records", key="SAGE-ADMIN-2026"))
...
# Initialize the local SQLite database for both development and WSGI servers.
# The database file is intentionally git-ignored for portfolio deployment.
initialize_database()
